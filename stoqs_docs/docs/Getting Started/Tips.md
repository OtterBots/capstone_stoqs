...
By default print line statements in python files will not be printed to docker logs or the console. In order to get print statements for debugging purposes change we need to set the environment variable `PYTHONUNBUFFERED` to 1. This can be done in the Dockerfile-stoqs. After enabling rebuild the containers with `docker-compose -f local.yml build`

``` yml line_nums="1" hl_line="7"
