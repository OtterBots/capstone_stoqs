...
class ActivityView(object):
    ...
    def generateActivityMapFile(self, template='activity.map'):
        ...
        if 'mappath' in self.request.session:
            ...
            self.request.session['mappath'] = __name__ + '_' + \
                ''.join(random.choices(string.ascii_uppercase + string.digits, k=10)) + '.map'
    ...
    def process_request(self, webPageTemplate='activitiesWMS.html', mapfile='activity.map'):
        ...
        return render(self.request, webPageTemplate, context={'mapserver_host': settings.MAPSERVER_HOST,
                                                              'list': self.itemList,
