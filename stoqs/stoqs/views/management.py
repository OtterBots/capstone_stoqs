...
def showDatabase(request):
    ...
    return render(request, 'management.html', context={'dbAlias': request.META['dbAlias'],
                                                       'actList': actList,
                                                       'pList': pList,
                                                       }
                  )
...
def showActivitiesMBARICustom(request):
    ...
    for a in aList:
        ...
        for mp in mparms:
            ...
            parmURL = "measurement/%s/between/%s/%s/%s" % (mp, a.startdate.strftime(
                '%Y%m%dT%H%M%S'), a.enddate.strftime('%Y%m%dT%H%M%S'), depthQuery)
