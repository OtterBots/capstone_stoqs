...
class BaseOutputer(object):
    ...
    def __init__(self, request, fmt, query_set, stoqs_object=None):
        ...
        self.html_tmpl_path = tempfile.NamedTemporaryFile(
            dir='/tmp', prefix=self.stoqs_object_name+'_', suffix='.html').name
    ...
    def build_html_template(self):
        ...
        '''
        response = render(self.request, self.html_tmpl_file, context={
    ...
    def process_request(self):
        ...
        if self.format == 'csv' or self.format == 'tsv':
            ...
            kml = KML(self.request, self.qs, self.qparams, self.stoqs_object_name,
                      withTimeStamps=False, withLineStrings=False, withFullIconURL=False)
            return kml.kmlResponse()
...
def showActivityResource(request, fmt='html'):
    ...
    o = BaseOutputer(request, fmt, query_set, stoqs_object)
    o.fields = ['id', 'uuid', 'activity__id', 'resource__id', 'activity__name',
                'resource__name', 'resource__value', 'resource__uristring']
    return o.process_request()
...
def showSimpleDepthTime(request, fmt='html'):
    ...
    o = BaseOutputer(request, fmt, query_set, stoqs_object)
    o.fields = ['id', 'activity__name', 'activity__activitytype__name',
                'nominallocation', 'instantpoint__timevalue', 'epochmilliseconds', 'depth']
    return o.process_request()
